...
class MarginalIndependentMultivariateNormalDistribution(Distribution): 
    def __init__(self, dim):
        super(MarginalIndependentMultivariateNormalDistribution, self).__init__()
        self.dim = dim
       
    def sampling(self, num_samples):
        return multivariate_normal.rvs(size=num_samples * self.dim).reshape(-1,self.dim)
    def density(self, sample_X):
        return multivariate_normal.pdf(sample_X, mean=np.zeros(self.dim), cov=np.diag(np.ones(self.dim)))
    
    
class ExponentialDecayMultivariateNormalDistribution(Distribution): 
    def __init__(self, dim, beta = 1 / 2):
        super(ExponentialDecayMultivariateNormalDistribution, self).__init__()
        self.dim = dim
        self.beta = beta
       
    def sampling(self, num_samples):
        cov = np.array([[self.beta**(np.abs( i - j)) for i in range(self.dim)] for j in range(self.dim)])
        return multivariate_normal.rvs(cov = cov, size=num_samples )
    def density(self, sample_X):
        cov = np.array([[self.beta**(np.abs( i - j)) for i in range(self.dim)] for j in range(self.dim)])
        return multivariate_normal.pdf(sample_X, mean=np.zeros(self.dim), cov=cov)
